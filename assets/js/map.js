...
class TitanMap {
  ...
  getCountryColor(stats) {
    const hasVerifiedLead = stats.leads.some(l => l.citizenship_sm === 'confirmed' && l.fifa_eligibility === 'verified');
    if (hasVerifiedLead) return '#d4af37';
    const hasActiveComm = stats.communities.some(c => c.status === 'active_dialogue' || c.status === 'primary_amplifier');
  ...
  colorCountries() {
    this.svg.querySelectorAll('.country-path').forEach(p => p.setAttribute('fill', '#1e2536'));
  ...
  proj(lat, lng) {
    return { x: (lng + 180) * (this.svgWidth / 360), y: (90 - lat) * (this.svgHeight / 180) };
  }
  ...
  pinRadius(list) {
    if (list.some(c => c.status === 'primary_amplifier')) return 10;
    if (list.some(c => c.status === 'active_dialogue')) return 8;
    return 6;
  }

  pinClass(list) {
    if (list.some(c => c.status === 'primary_amplifier')) return 'pin-amplifier';
    if (list.some(c => c.status === 'active_dialogue')) return 'pin-active';
    return 'pin-pending';
  }
  ...
  drawPins() {
    ...
    for (const [, group] of Object.entries(cityGroups)) {
      ...
      pulse.setAttribute('r', r + 2); pulse.setAttribute('class', 'pin-pulse'); pulse.setAttribute('fill', 'transparent');
    ...
    smG.setAttribute('data-country', 'SM'); smG.setAttribute('data-city', 'San Marino'); smG.style.cursor = 'pointer';
  ...
  attachEvents() {
    this.svg.querySelectorAll('.country-path').forEach(path => {
      ...
      if (!id) return;
      path.addEventListener('mouseenter', e => this.onCountryHover(e, id));
    ...
    this.svg.querySelectorAll('.pin-group').forEach(pin => {
      ...
      pin.addEventListener('click', e => { e.stopPropagation(); PanelController.openCountry(country, this.countryStats, this.data, city); });
    });
  ...
  onCountryHover(e, id) {
    ...
    if (!stats) return;
    const nComm = stats.communities.length, nLead = stats.leads.length;
    if (!nComm && !nLead) return;
    this.tooltip.innerHTML = `<div class="tooltip-country">${countryFlag(id)} ${id}</div><div class="tooltip-stats">${nComm?`<span class="tooltip-stat">🏙 ${nComm} comunità</span>`:''} ${nLead?`<span class="tooltip-stat">⚽ ${nLead} lead</span>`:''}</div>`;
  ...
  onPinHover(e, country, city) {
    ...
    this.tooltip.innerHTML = `<div class="tooltip-country">${countryFlag(country)} ${city || country}</div><div class="tooltip-stats">${comms.length?`<span class="tooltip-stat">🏙 ${comms.length} comunità</span>`:''} ${leads.length?`<span class="tooltip-stat">⚽ ${leads.length} lead</span>`:''}</div>`;
    this.tooltip.style.display = 'block';
    this.onTooltipMove(e);
  }
